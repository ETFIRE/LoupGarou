...
class GameManager:
    ...
    def night_phase(self):
        """Exécute la phase de nuit."""
        self.log(f"🌙 Nuit {self.day + 1}")
        alive = self.get_alive_players()
    ...
    def voting_phase(self, human_vote_target=None):
        ...
        for voter in [p for p in alive if not p.is_human]:
            vote = voter.decide_vote(self._get_public_status(), "Débat terminé")
    ...
    def execute_lynching(self):
        """Exécute le lynchage."""
        if not self.vote_counts:
            self.log("Aucun vote, personne n'est lynché")
            return None
        ...
        if len(candidates) > 1:
            self.log(f"Égalité entre {', '.join(candidates)}. Personne n'est lynché.")
            return None
        ...
        if target and target.is_alive:
            ...
            self.log(f"🔥 {target.name} est lynché ! Rôle: {target.role.name}")
            return target
