...
# --- CLASSE GAMEMANAGER (VERSION COMPLÈTE) ---
...
class GameManager:
    ...
    def _distribute_roles(self, custom_roles=None):
        """Distribue aléatoirement les rôles aux joueurs et assigne les personnalités aux IA"""
        ...
        for player in self.players:
            ...
            # Si c'est une IA (ChatAgent), on lui assigne une personnalité
            if not player.is_human:
                # 1. Tirer une personnalité en fonction du rôle (biaisé mais pas déterministe)
                personality = pick_personality_for_role(role.name)

                # 2. Mettre à jour son chemin de contexte
                player.personality_context_path = personality.context_path

                # 3. Réinitialiser son historique avec la nouvelle personnalité
                player.initiate_history()  # relit le fichier de personnalité

                # 4. Ajouter une couche de contexte sur le rôle précis + personnalité
        ...
        for voyante in ordered_actions[NightAction.INVESTIGATE]:
            if not voyante.is_human:
                ...
                if target:
                    ...
                    print(
                        f"🔮 La Voyante ({voyante.name}) a enquêté sur {target.name}."
                    )
    ...
    def _voting_phase(self, alive_players):
        ...
        for voter in alive_players:
            if not voter.is_human:
                ...
                # TODO: remplacer par un input réel joueur (UI) au lieu de random
